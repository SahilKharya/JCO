...
describe("Foundation Contract", function () {
    ...
    describe("Deployment", function () {
        it("Should assign the manager", async function () {
            expect(await foundation.manager()).to.equal(jco_manager.address);
        });

        it("Should set the number of confirmations required", async function () {
            expect(await foundation.numConfirmationsRequired()).to.equal(2);
        });
    ...
    describe("Transactions", function () {
        it("Should submit transaction successfully", async function () {
            const tx = await jco_manager.submitTxn_Foundation(account1.address, 100, "0x");
            await tx.wait();
        ...
        it("Should confirm transaction successfully", async function () {
            await jco_manager.submitTxn_Foundation(account1.address, 100, "0x");
